...
class NIDBackService:
    ...
    def process(self, image_path: str) -> NIDBackData:
        ...
        try:
            ...
            logger.info(f"Back OCR: {len(all_segments)} segments collected")

            fmt = self._detector.detect(all_segments)
            fields = self._extractor.extract(all_segments, fmt)

            # mrz_name from back extractor is informational — not part of NIDBackData
